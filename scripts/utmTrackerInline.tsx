...
(function () {
    ...
    if (isAdClick) {
        ...
        const bid = getParameterByName('bid');
        ...
        if (ref) setCookie('cookieRef', ref, 63072000);
        if (bid) setCookie('cookieBid', bid, 63072000);
    }

    // Coletar valores das UTMs e cookies para manipulação dos links
    let parametros = ["utm_source", "utm_medium", "utm_campaign", "utm_content", "utm_term", "ref", "bid"];
    ...
    const cookieRef = getCookieValue('cookieRef');
    const cookieBid = getCookieValue('cookieBid');
    ...
    // Coletar todos os valores para o parâmetro sck
    let scks = Object.values(utms).filter(value => value !== "");

    // Capturar o parâmetro ref se existir
    const refValue = urlParamsCapt.get('ref') || '';
    if (refValue && !scks.includes(refValue)) {
        scks.push(refValue);
    }

    // Capturar o parâmetro bid se existir
    const bidValue = urlParamsCapt.get('bid') || '';
    if (bidValue && !scks.includes(bidValue)) {
        scks.push(bidValue);
    }

    // Verificar se já existe um parâmetro sck na URL
    let currentSckValues = [];
    if (urlParamsCapt.get('sck')) {
        currentSckValues = urlParamsCapt.get('sck').split('|');
        // Adicionar valores existentes do sck ao array scks
        currentSckValues.forEach(value => {
            if (!scks.includes(value)) {
                scks.push(value);
            }
        });
    }

    // Valores para o parâmetro src
    let srcValues = [cookieUtmSource, cookieUtmMedium, cookieUtmCampaign, cookieUtmContent, cookieUtmTerm, cookieRef, cookieBid].filter(value => value !== null && value !== "");
    ...
    const updateLinks = (el, elURL) => {
        try {
            ...
            if (cookieRef) elSearchParams.append('cookieRef', cookieRef);
            if (cookieBid) elSearchParams.append('cookieBid', cookieBid);
            ...
            if (elURL.hostname.includes('hotmart.com')) {
                ...
                // Adicionar ref se não existir e tiver um valor de cookie ou URL
                if (!elSearchParams.has('ref') && (cookieRef || refValue)) {
                    elSearchParams.append('ref', cookieRef || refValue);
                    modified = true;
                }
                
                // Garantir que o sck seja adicionado aos links da Hotmart
                if (!elSearchParams.has('sck') && scks.length > 0) {
                    elSearchParams.append('sck', scks.join('|'));
                    modified = true;
                }
                
                // Adicionar bid se não existir e tiver um valor
                if (!elSearchParams.has('bid') && (cookieBid || bidValue)) {
                    elSearchParams.append('bid', cookieBid || bidValue);
                    modified = true;
                }
            }
    ...
    // Função para processar todos os links
    const processAllLinks = () => {
        document.querySelectorAll('a').forEach(el => {
            try {
                ...
                // Verificar se é um link da Hotmart
                const isHotmartLink = elURL.hostname.includes('hotmart.com');
                
                // Processar apenas links da Hotmart ou se não for um link com hash
                if (isHotmartLink || !elURL.hash) {
                    el.href = updateLinks(el, elURL);
                    
                    // Adicionar log para depuração em links da Hotmart
                    if (isHotmartLink) {
                        console.log('Link da Hotmart processado:', el.href);
                    }
                }
            } catch (e) {
